...
import com.tvroom.downloader.BuildConfig;
...
public final class TelegramActivationClient {
    ...
    public void notifyActivated(long replyToMessageId) {
        Thread confirmation = new Thread(() -> sendMessage(
                "✅ [티비룸 · " + userName + "] 영구 활성화되었습니다.",
                replyToMessageId), "tvroom-activation-confirmation");
        confirmation.start();
    }
            ...
            try {
                ...
                if (detail.contains("409")) {
                    listener.onConnectionMessage(
                            "같은 텔레그램 봇을 확인 중인 다른 프로그램을 종료해 주세요.");
                } else {
    ...
    private void processUpdate(JSONObject update) {
        ...
        if (code.isEmpty()) {
            sendMessage("활성화 암호가 비어 있습니다. 예: /티비룸 " + userName + " (1234)",
                    messageId);
            return;
        }
        if (!target.equals(userName)) return;

        ActivationStore.setPendingCode(context, code, messageId);
        sendMessage("🔒 [티비룸 · " + userName + "] 암호 입력 대기 중…", messageId);
    ...
    private JSONObject getUpdates(long offset, int timeoutSeconds) throws Exception {
        ...
        try {
            ...
            if (status < 200 || status >= 300) {
                throw new IllegalStateException("Telegram HTTP " + status);
            }
            JSONObject result = new JSONObject(readText(connection.getInputStream()));
            if (!result.optBoolean("ok", false)) {
                throw new IllegalStateException("Telegram API 오류");
            }
            return result;
        } finally {
    ...
    private HttpURLConnection open(URL url, String method) throws Exception {
        ...
        connection.setRequestProperty("User-Agent", "TVRoomDownloader/1.1");
        return connection;
    }
